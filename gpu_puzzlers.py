# -*- coding: utf-8 -*-
"""my_solutions_to_gpu_puzzlers.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1jGyG_Iy3uG78ohoI8RcipLckP3-26CLi
...
def pool_test(cuda):
    def call(out, a, size) -> None:
        ...
        if i < size:
          shared[local_i] = a[i] # read a into shared memory
          cuda.syncthreads()
...
def conv_test(cuda):
    def call(out, a, b, a_size, b_size) -> None:
        ...
        if i < a_size:
          ...
          if local_i < b_size - 1 and i + TPB < a_size: # gets first few threads to make a wrap around to get extra ones at end
        ...
        if i < a_size:
          ...
          for j in range(b_size):
            if i + j < a_size:
              val += shared_a[local_i + j] * shared_b[j]
          out[i] = val
...
def axis_sum_test(cuda):
    def call(out, a, size: int) -> None:
        ...
        if local_i == 0: # one thread per block
          result = 0.0
          for k in range(TPB):
            result += cache[k]
          out[batch, cuda.blockIdx.x] = result
...
def mm_oneblock_test(cuda):
    def call(out, a, b, size: int) -> None:
        ...
        result = 0.0
        for tile_idx in range(0, size, TPB):
          # load tile from global -> shared
          if i < size and (tile_idx + local_j) < size:
            a_shared[local_i, local_j] = a[i, tile_idx + local_j]
          else:
            a_shared[local_i, local_j] = 0.0

          if j < size and (tile_idx + local_i) < size:
            b_shared[local_i, local_j] = b[tile_idx + local_i, j]
          else:
          ...
          for k in range(TPB):
            result += a_shared[local_i, k] * b_shared[k, local_j]
